...
function set_initial_state() {
    ...
    */
    resources = (_a = {
        ...
        _a["hydrogen"] = { "amount": 0, get value() { if (adventure_data["challenge"] == CHALLENGES.UDM) {
                return 1;
            } return 5; }, "mult": 1, "changes": {}, "ps_change": "" },
...
var prestige = {
    ...
    mana: function (round) {
        ...
        var mana = Math.abs(buildings["s_manastone"].amount - mana_this_prestige); /* Don't count mana gained this prestige in here. */
...
function add_log_elem(to_add, type) {
    if (type === void 0) { type = ""; }
    if (true) {
...
function update() {
    ...
    Object.keys(resources).forEach(function (key) {
        ...
        /* Omnivision lets them see resource amounts */
        if (event_flags["skills"] && event_flags["skills"][7]) {
...
function update_upgrade_list() {
    ...
    Object.keys(remaining_upgrades).forEach(function (upg_name) {
        ...
        if (remaining_upgrades[upg_name].unlock()) {
            ...
            if (adventure_data["auto_upgrade"] && color_1 == "" && !remaining_upgrades[upg_name].repeats && remaining_upgrades[upg_name].cost["time"] == undefined) {
                try {
...
function draw_autobuild() {
    ...
    var _loop_1 = function (i) {
        var building = build_queue[i];
        var build_amount = 1;
        if (typeof building == "object") {
            build_amount = building[1];
            building = building[0];
        }
        var b_name = $("#building_" + building + " .building_name").text();
    ...
    if (event_flags["start_buildings"] == undefined) {
        ...
        if (adventure_data["challenges_completed"]) { /* This has been defined */
            ...
            if (adventure_data["challenges_completed"].length >= CHALLENGES.DISCO && adventure_data["challenges_completed"][CHALLENGES.DISCO]) {
                /* Make sure it's defined to not get fuzzy production. */
                if (buildings["s_challenge"].generation["uranium"] == undefined) {
                    buildings["s_challenge"].generation["uranium"] = 0;
                }
