...
function s_trade(delta_time: number) {
    ...
    if (to_next_trade < 0 && !remaining_upgrades["trade"].unlock()) {
        ...
        let money_value = Math.round(Math.max(1, Math.random() * Math.min(Math.pow(Math.min(resources["mana"].amount, 10000), 3) * 10, resources["money"].amount) * 2 + 10));
        ...
        let value_cap = 100 + (purchased_upgrades.indexOf("better_trades") != -1 ? 50 : 0) + (purchased_upgrades.indexOf("better_trades_2") != -1 ? 50 : 0);
...
let enchantments = {
    "money": {
        ...
        "effect": function () {
            ...
            let build_state = buildings["s_enchantment"].on;
            if (build_state) {
...
function s_enchant_set(enchant: string) {
...
function s_enchant_update(delta_time: number) {
    /* Tick building */
...
function s_final(delta_time: number) {
    ...
    Object.keys(resources).forEach((res) => {
        ...
        let normal_gain = 0;
        let neg_gain = 0;
        Object.keys(buildings).forEach((build) => {
        ...
        if (normal_gain != 0) {
            this["res-" + res] = Math.max(0, normal_gain * buildings["s_final"].strength);
